...
bool CChat::Init()
{
	...
	btnshow->evtMouseClick	=	_evtQQMainEditMotto;
	//	End
...
void CChat::_OnMouseFrndGroupMenu(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	else if (str=="删除分类")
	{
		...
		{
			char buffer[255];
			sprintf(buffer,"该分组已经有%d 个成员，不能删除",count);
...
void CChat::ShowDetailForm(CMember* pMember)
{
	if(pMember)
	{
...
void CChat::_OnMouseMasterMenu(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	else if(strCommand == RES_STRING(CL_LANGUAGE_MATCH_854))	// 解除关系（徒弟解除师傅）
	{
		...
		if( pMain && pMain->getGameAttr() )
		{
			...
			char szBuffer[256] = {0};
			sprintf(szBuffer, RES_STRING(CL_LANGUAGE_MATCH_889), _curSelectMember->GetName(), nLevel * 500); // "解除师傅 %s 您将扣除\n金钱:%d\n确定吗？"
...
void CChat::_OnMouseStudentMenu(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	else if(strCommand == RES_STRING(CL_LANGUAGE_MATCH_854))	// 解除关系（师傅解除徒弟）
	{
		...
		if( pMain && pMain->getGameAttr() )
		{
			...
			char szBuffer[256] = {0};
			sprintf(szBuffer, RES_STRING(CL_LANGUAGE_MATCH_856), _curSelectMember->GetName(),nLevel * 5); // 解除徒弟 %s 您将扣除\n金钱:%d  声望:%d\n确定吗？
...
void CChat::_evtChangeMotto(CGuiData *pSender, int x, int y, DWORD key)
{
	...
	}else if(pSender == pbtnAdd1)  // 鸡蛋
	{
		...
		char strcontent[128]="";
		sprintf(strcontent,"%d",content+1);
...
bool CChat::_UpdateSelfInfo()
{
	...
	//玩家名称
	sprintf( ChCharaterName , "%s", pCha->getName());

	sprintf(ChCharaterNickName,"%s",	pCChaAttr->get( ATTR_TITLE));
	...
	//玩家职业
	sprintf(ChCharaterJob,"%s",	g_GetJobName((short)pCChaAttr->get(ATTR_JOB)));
		
	//玩家等级
	sprintf( ChCharaterLev , "%d", pCChaAttr->get(ATTR_LV));				 
		
	// 声望
	sprintf(ChCharaterFame, "%d", pCChaAttr->get(ATTR_FAME));
...
void CChat::ResetSelfInfo(const stPersonInfo *_info )
{
	...
	//sprintf(buff,"%d",(_info->iBirthday &&0xffff0000));
	//m_pBirthMonth->SetCaption(buff);
	//sprintf(buff,"%d",(_info->iBirthday &&0xffff));
	//Modify by sunny.sun 20080825
	sprintf(buff,"%d",(_info->iBirthday >> 5));
	m_pBirthMonth->SetCaption(buff);
	sprintf(buff,"%d",(_info->iBirthday - (_info->iBirthday >> 5 << 5)));
