...
void CSelectChaScene::_RenderUI()
{
    ...
    {		
        ...
        string sSecond = string("Lv") + string(itoa((*iter)->iLevel, szBuf, 10)) + " " + (*iter)->sProfession;
        ...
        CGuiFont::s_Font.TipRender( (sFirst + "\n" + sSecond).c_str(), (*iter)->iFontX, (*iter)->iFontY );
    }
}
...
bool CSelectChaScene::_MouseButtonDown(int nButton)	
{
    ...
    //判断鼠标是否点中人物
    CCharacter* pCha = this->HitTestCharacter(g_pGameApp->GetMouseX(), g_pGameApp->GetMouseY());
    if (NULL == pCha)
    {
        return false;
    }
    ...
    {
        m_CharactorPtrs[m_nCurChaIndex]->pCha->PlayPose( 1, PLAY_LOOP, -1, CGameApp::GetFrameFPS(), true);
    ...
    m_CharactorPtrs[m_nCurChaIndex]->pCha->PlayPose(2, PLAY_LOOP, -1, CGameApp::GetFrameFPS(), true);
    m_CharactorPtrs[m_nCurChaIndex]->pCha->SetColor(m_ChaColors[m_nCurChaIndex][0],m_ChaColors[m_nCurChaIndex][1], m_ChaColors[m_nCurChaIndex][2]);
...
bool CSelectChaScene::_InitUI()
{
    ...
    frmSelectCha->SetPos((g_pGameApp->GetWindowWidth() - frmSelectCha->GetWidth())/2,g_pGameApp->GetWindowHeight() - frmSelectCha->GetHeight() - 20 );
...
//-----------------------------------------------------------------------
void CSelectChaScene::_SelChaFrmMouseEvent(CCompent *pSender, int nMsgType, int x, int y, DWORD dwKey)
{
    ...
    if(strName=="btnCreate")
    {
        ...
        CCreateChaScene* pkScene = dynamic_cast<CCreateChaScene*>(g_pGameApp->CreateScene(&param));
        if (!pkScene) return;
