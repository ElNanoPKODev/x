...
{
	const char* text = g_ResourceBundleManage.LoadResString(id);
	return text;
}
...
int UI_SetCaptionEx( int id, char* caption, int tail)
{
	...
	char buffer[255];
	sprintf(buffer,"%s%d", caption, tail);

	p->SetCaption( buffer );
	return R_OK;
}

const int AddMisDataFromC(int id, char* name, int type)
{
	CLU_CallScriptFunction( "AddMisData", "int", "ushort, char*, ushort", id, name, type);
	return R_OK;
}
...
void MPInitLua_Gui()
{
	...
	CLU_RegisterFunction("AddMisDataFromC", "ushort", "int,char*", CLU_CDECL, CLU_CAST(AddMisDataFromC));
}
