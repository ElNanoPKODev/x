...
pi_LeakReporter::~pi_LeakReporter(void)
{
	...
	if(b>0 || y>0)
	{
		cerr << "pi :MEMORY LEAK --" << b << " block" << (b!=1?"s":"") << "," << y << " byte" << (y!=1?"s":"") << endl;
	}
	dumpReport();
}
...
void pi_LeakReporter::dumpReport()
{
	if(pi_leakReporter.pi_leakList.size() > 0)
	{
		...
		{
			cout << "\t" << (i+1) << ":" << it->fileName<< " size=" << ((it->d_size > 1024) ? (it->d_size/1024.0) : it->d_size) << ((it->d_size > 1024) ? " kbytes (" : " bytes(") << it->d_lineNo << ")" << endl;
		}
		...
		try
		{
			...
			for(pi_LeakList::iterator it = pi_leakReporter.pi_leakList.begin(); it!=pi_leakReporter.pi_leakList.end();it++, i++)
			{
				outputFile << "\t" << (i+1) << ":" << it->fileName<< " size=" << ((it->d_size > 1024) ? (it->d_size/1024.0) : it->d_size) << ((it->d_size > 1024) ? " kbytes (" : " bytes(") << it->d_lineNo << ")" << endl;
			}
