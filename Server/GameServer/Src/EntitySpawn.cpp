		...
		for (int i = 0; i < m_lRecordNum; i++)
 		{
			...
			pCItem = pCMap->ItemSpawn(&SItemCont, pCSwitchMapRecord->SEntityPos.x, pCSwitchMapRecord->SEntityPos.y, enumITEM_APPE_NATURAL, 0, g_pCSystemCha->GetID(), g_pCSystemCha->GetHandle(), -1, -1,&CEvtCont);
			if (pCItem)
				pCItem->SetOnTick(0);
		}
	}
